...
{
	CSMRendering::CSMRendering(RenderDeviceD3D12* device, TessellationPSOData* psoData) :
		m_Device(device),
		m_PsoData(psoData)
	{
		...
		for (int i = 0; i < CascadeCount; i++)
		{
			...
			swprintf(bufferName, 16, L"ShadowMap-%d", i);
			m_ShadowMaps[i]->SetName(bufferName);
	...
	void CSMRendering::Render(Camera* camera, Light* light, const Timer& timer, const AdaptiveTessellationCompute* adaptiveTessellation)
	{
		...
		for (int i = 0; i < CascadeCount; i++)
		{
			float cascadeFar = CSMSplits[i] * ShadowDistance;
			...
			TessellationShadowMapPass::PassConstants passConstants;
			...
			passConstants.CamPosition = camera->GetPosition(); // TODO: camera or light position?
